...
namespace Vexe.Editor.Framework
{
	public class MemberDrawersHandler
	{
		...
		{
			cachedAttributeDrawers = new Dictionary<MemberInfo, BaseDrawer>();
			cachedCoreDrawers = new Dictionary<MemberInfo, BaseDrawer>();
			cachedMethodDrawers = new Dictionary<MemberInfo, BaseDrawer>();
			cachedCompositeDrawers = new Dictionary<MemberInfo, List<BaseDrawer>>();
		...
		public List<BaseDrawer> GetCompositeDrawers(MemberInfo member, Attribute[] attributes, bool coreOnly)
		{
			...
			if (cachedCompositeDrawers.TryGetValue(member, out drawers))
				return drawers;
			...
			cachedCompositeDrawers.Add(member, drawers);
			return drawers;
		}
		...
		public BaseDrawer GetMemberDrawer(MemberInfo member, Attribute[] attributes, bool coreOnly)
		{
			if (member.MemberType == MemberTypes.Method)
				return GetMethodDrawer(member);

			Dictionary<MemberInfo, BaseDrawer> cache = coreOnly ?
				cachedCoreDrawers : cachedAttributeDrawers;
			...
			for (int i = 0; i < resolvers.Length; i++)
				if ((drawer = resolvers[i].Resolve(memberType, attributes, coreOnly)) != null)
					break;
		...
		{
			BaseDrawer drawer;
			if (!cachedMethodDrawers.TryGetValue(method, out drawer))
				cachedMethodDrawers.Add(method, drawer = new MethodDrawer());
			return drawer;
		}

		public BaseDrawer GetObjectDrawer(Type objectType, bool coreOnly)
		{
			return GetDrawerForType(objectType, coreOnly ?
					objectDrawerTypes.Where(t => t.IsDefined<CoreDrawerAttribute>(true)).ToArray() :
					objectDrawerTypes, typeof(ObjectDrawer<>),
				() => BaseDrawer.Create(fallbackDrawerType.MakeGenericType(objectType)));
		}
		...
		private class AttributeDrawerResolver : DrawerResolver
		{
			public override BaseDrawer Resolve(Type memberType, IEnumerable<Attribute> attributes, bool coreOnly)
			{
				...
				return handler.GetAttributeDrawer(memberType, drawingAttribute.GetType());
			}
		}
		...
		private class ObjectDrawerResolver : DrawerResolver
		{
			public override BaseDrawer Resolve(Type memberType, IEnumerable<Attribute> attributes, bool coreOnly)
			{
				return handler.GetObjectDrawer(memberType, coreOnly);
			}
		}
		...
		private struct TypePair
		{
			...
			public override bool Equals(object obj)
			{
				try
				{
					...
					return pair.type1 == type1 && pair.type2 == type2;
				}
				catch
				{
					return false;
